...
func main() {
	...
	flag.StringVar(&parameters.admCfgFile, "admCfgFile", "/etc/webhook/config/admconfig.yaml", "File containing the mutation configuration.")
	flag.Parse()

	var admConfig patchv1beta1.Conf
	err := admConfig.LoadConfig(parameters.admCfgFile)
	glog.Infof("loadconfig - allowed fs: %v", admConfig.AllowedFileSystem)
	if err != nil {
